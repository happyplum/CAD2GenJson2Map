...
export async function buildObstacleGraph(geojson, options = {}) {
  ...
  // 提取障碍物多边形
  const obstacles = includeObstacles
    ? extractObstaclesFromGeoJSON(geojson, options)
    : [];
  ...
  const obstaclesMeta = obstacles.map((rings) => ({
    rings,
